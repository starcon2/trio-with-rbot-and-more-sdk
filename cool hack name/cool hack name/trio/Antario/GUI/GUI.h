...
namespace ui
{
    ...
    struct MenuStyle
    {
        int   iPaddingX       = 12;                     /*- Padding between sections                            -*/
        int   iPaddingY       = 12;                     /*- Padding between selectables                         -*/
        ...
        Color colSectionOutl  = { 1, 1, 1, 255 };       /*- Color of the section outline                        -*/
		...
		Color colTextOut      = { 100, 100, 100, 255 }; /*- Color of the text when always on is enabled         -*/
    ...
    {
    public:
        Window() : bIsDragged(false), iHeaderWidth(0), tSelectedTab(nullptr), pHeaderFont(nullptr) { }
        ...
        int  GetHeaderWidth() const { return this->iHeaderWidth; }

    private:
        bool   bIsDragged;     /* Says if the window is currently dragged. */
        int    iHeaderWidth;   /* Header height in pixels                  */
    ...
    public:
		...
		virtual void AddList(const std::string& strSelectableLabel, int iSize, int* iVecIndex, std::vector<std::string> vecBoxOptions);
		virtual void AddMulti(const std::string& strSelectableLabel, bool * bEnabled, std::vector<std::string> vecBoxOptions);
		virtual void AddSlider(const std::string& strLabel, std::string strAddon, float* flValue, float flMinValue, float flMaxValue);
		virtual void AddSlider(const std::string& strLabel, std::string strAddon, int* iValue, int iMinValue, int iMaxValue);
		virtual void AddKeyBind(const std::string& strSelectableLabel, ButtonCode_t * key, int * iCurrentValue, SPoint ptButtonSize = SPoint(0, 0));
		virtual void AddColor(const std::string& strLabelName, Color* colorOutput);
		virtual void AddTextField(const std::string& strSelectableLabel, std::string * strInput, SPoint ptButtonSize = SPoint(0, 0));
		virtual void AddText(const std::string& strLabel);
    protected:
	...
	class KeyPicker : public Control
	{
	public:
		KeyPicker(const std::string& strLabel, ButtonCode_t * key, int * iCurrentValue, ObjectPtr pParent, SPoint ptKeyPickerSize = SPoint(0, 0));
		void Render()                                                       override;
		void Initialize()                                                   override;
		bool UpdateData();
		bool HandleMouseInput(UINT uMsg, WPARAM wParam, LPARAM lParam)      override;
	...
	private:
		...
		bool     bIsActivated;            /* Defines if we activated the KeyPicker                         */
		bool     bPressed;                /* if mouse is pressed                                           */
	...
	{
	public:
		TextField(const std::string& strLabel, std::string * strInput, ObjectPtr pParent, SPoint ptKeyPickerSize = SPoint(0, 0));
		void Render()                                                       override;
		void Initialize()                                                   override;
		bool UpdateData();
		bool HandleMouseInput(UINT uMsg, WPARAM wParam, LPARAM lParam)      override;
	...
	private:
		float blink;
		std::string * input;
		bool     bIsActivated;            /* Defines if we activated the textfield                         */
		bool     bPressed;                /* if mouse is pressed                                           */
		SPoint   ptSelectablePosition;    /* Position of the selectable                                    */
		SPoint   szSelectableSize;        /* Size of the internal selectable size of the textfield         */
		SRect    rcDelete;
	};
    ...
    private:
		...
		SPoint szSelectableSize;    /* Size of the internal selectable size of the button            */
    };

	class ListBox : public Control
	{
	public:
		ListBox(const std::string& strLabel, std::vector<std::string> vecBoxOptions, int iSize, int* iCurrentValue, ObjectPtr pParent);
		void Initialize()         override;
		void Render()             override;
		bool HandleMouseInput(UINT uMsg, WPARAM wParam, LPARAM lParam)      override;
		bool CanHaveFocus() const override { return true; }
    ...
    {
	public:
		Slider(const std::string& strLabel, std::string& strAddon, T* tValue, T tMinValue, T tMaxValue, ObjectPtr pParent);
	...
	private:
		...
		std::string strAddon; /* thing of what slider is doing                     */
		SRect rcMinus;
		SRect rcPlus;
	};
