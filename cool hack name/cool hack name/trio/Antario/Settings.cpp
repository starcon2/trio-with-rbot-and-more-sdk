...
void Settings::Initialize(MenuMain* pMenuObj)
{
    ...
    fsDefaultPath.append("default.cfg");
...
void Settings::SaveSettings(const std::string& strFileName, MenuMain* pMenuObj)
{
    ...
    loopChildSettings = [&loopChildSettings](MenuMain* pMenuObj/*, Parent/section pointer for sections in cfgFile*/) -> void
    {
        ...
        for (auto& it : pMenuObj->vecChildren)
        {
			...
			case MST::TYPE_COLOR:
				{
				auto tmpColor = dynamic_cast<ColorPicker*>(it.get());

				break;
				}
			case MST::TYPE_KEYPICKER:
				{
					auto tmpKeyBind = dynamic_cast<KeyPicker*>(it.get());

					break;
				}
			case MST::TYPE_MULTI:
				{
					auto tmpMulti = dynamic_cast<MultiBox*>(it.get());

					break;
				}
			case MST::TYPE_SLIDER:
				{
					auto tmpSliderInt = dynamic_cast<Slider<int>*>(it.get());
