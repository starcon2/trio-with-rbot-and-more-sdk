...
void DrawManager::InitDeviceObjects(LPDIRECT3DDEVICE9 pDevice)
{
	...
	// Create new fonts
	g_Fonts.pFontTahoma8 = std::make_unique<CD3DFont>(L"Tahoma", 7, 550);
	g_Fonts.pFontTahoma6 = std::make_unique<CD3DFont>(L"Tahoma", 6, 550);
	g_Fonts.pFontTahoma10 = std::make_unique<CD3DFont>(L"Tahoma", 10, FW_MEDIUM);
	g_Fonts.pFontTabs = std::make_unique<CD3DFont>(L"cherryfont", 35, FW_NORMAL);
	g_Fonts.pFontESPFont2 = std::make_unique<CD3DFont>(L"smallest_pixel-7", 6, 100);
	g_Fonts.pFontIcons = std::make_unique<CD3DFont>(L"AstriumWep", 8, 100);
	g_Fonts.pFontESPFont = std::make_unique<CD3DFont>(L"smallest_pixel-7", 5, 100);
...
void DrawManager::Rect(int posx1, int posy1, int posx2, int posy2, Color color) const
{
    ...
    this->pDevice->SetRenderState(D3DRS_MULTISAMPLEANTIALIAS, FALSE);   /* And enable again for the rest of the drawing */
}

void DrawManager::OutlinedRect(int x, int y, int w, int h, Color color) const
{
	this->Rect(x, y, w + x, h + y, color);
}

void DrawManager::FilledRect(int x, int y, int w, int h, Color color) const
{
	this->RectFilled(x, y, w + x, h + y, color);
}
...
void DrawManager::String(SPoint vecPos, DWORD dwFlags, Color color, CD3DFont* pFont, std::string szText, ...) const
{
	pFont->DrawString(vecPos.x, vecPos.y, COL2DWORD(color), szText.c_str(), dwFlags);
}


void DrawManager::String(int posx, int posy, DWORD dwFlags, Color color, CD3DFont* pFont, std::string szText, ...) const
{
	pFont->DrawString(FLOAT(posx), FLOAT(posy), COL2DWORD(color), szText.c_str(), dwFlags);
}

void DrawManager::createSprite(LPDIRECT3DTEXTURE9 p_texture, byte* file, UINT size, int width, int height) 
{
	D3DXCreateTextureFromFileInMemoryEx(pDevice, file, size, width, height, D3DX_DEFAULT, 0, D3DFMT_UNKNOWN, D3DPOOL_MANAGED, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &p_texture);

	/* lock the texture */
	D3DLOCKED_RECT tex_locked;

	if (p_texture->LockRect(0, &tex_locked, nullptr, 0) != D3D_OK)
		return;
}

void DrawManager::Sprite(LPDIRECT3DTEXTURE9 p_texture, int x, int y, float rotation) 
{
	if (!pSprite)
	...
	if (p_texture != NULL) {
		...
		pSprite->Draw(p_texture, NULL, NULL, &D3DXVECTOR3(x, y, rotation), 0xFFFFFFFF);
		pSprite->End();
	}
}
